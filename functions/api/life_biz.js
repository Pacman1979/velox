export async function onRequest(context) {
  ...
  if (request.method === 'GET') {
    const data = await env.IDEAS_KV.get('life_biz');
    return new Response(data || '[]', { headers });
  }

  if (request.method === 'POST') {
    const body = await request.json();
    const existing = JSON.parse(await env.IDEAS_KV.get('life_biz') || '[]');
    existing.push({ id: crypto.randomUUID(), text: body.text, importance: body.importance || 5, done: false, ts: Date.now() });
    await env.IDEAS_KV.put('life_biz', JSON.stringify(existing));
  ...
  if (request.method === 'PATCH') {
    ...
    await env.IDEAS_KV.put('life_biz', JSON.stringify(updated));
